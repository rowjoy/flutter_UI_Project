// ignore_for_file: avoid_unnecessary_containers, prefer_const_constructors, sized_box_for_whitespace, unrelated_type_equality_checks

import 'package:flutter/material.dart';
import 'package:flutter_svg/flutter_svg.dart';

import 'package:seepups/controllers/sign_up/sign_up.dart';
import 'package:seepups/utils/su_colors.dart';
import 'package:seepups/utils/su_custom_appber.dart';
import 'package:seepups/utils/su_gbutton.dart';
import 'package:seepups/utils/su_svgimage.dart';
import 'package:get/get.dart';
import 'package:seepups/views/sign_up/su_signup_page.dart';

class SignUpaspage extends StatefulWidget {
  static const String path = 'Loginaspage';

  const SignUpaspage({Key? key}) : super(key: key);

  @override
  _SuLoginpageState createState() => _SuLoginpageState();
}

class _SuLoginpageState extends State<SignUpaspage> {
  final Signupas signupas = Get.put(Signupas());
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
      body: SafeArea(
          child: Container(
        child: Column(
          children: [
            Customappber(
              label: 'Sign Up',
            ),
            Expanded(
              child: Container(
                child: AspectRatio(
                  aspectRatio: 2,
                  child: Padding(
                    padding: const EdgeInsets.only(left: 20, right: 20),
                    child: SvgPicture.asset(Svgimage.signupasimage),
                  ),
                ),
              ),
            ),
            Container(
              width: MediaQuery.of(context).size.width,
              decoration: BoxDecoration(
                color: Appcolors.backgroundColor,
                // ignore: prefer_const_literals_to_create_immutables
                boxShadow: [
                  BoxShadow(
                    color: Colors.grey.withOpacity(0.5),
                    spreadRadius: 10,
                    blurRadius: 7,
                    offset: Offset(0, 5), // changes position of shadow
                  ),
                ],
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(30),
                  topRight: Radius.circular(30),
                ),
              ),
              child: Padding(
                padding: const EdgeInsets.only(
                    left: 10, right: 10, bottom: 8, top: 20),
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Container(
                      child: Text(
                        ' Sign Up as',
                        style: TextStyle(
                          color: Appcolors.normalfontcolor,
                          fontSize: 18,
                        ),
                      ),
                    ),
                    SizedBox(
                      height: 20,
                    ),
                    Container(
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          userselected(1, Svgimage.investorimage, 'Investor'),
                          userselected(2, Svgimage.startupimage, 'Startup'),
                        ],
                      ),
                    ),
                    SizedBox(
                      height: 20,
                    ),
                    SuGButton(
                      onPressed: () {
                        Get.to(() => const Signuppage());
                      },
                      title: 'Next',
                    ),
                  ],
                ),
              ),
            ),
          ],
        ),
      )),
    );
  }

  Widget userselected(dynamic index, String assetName, String data) {
    return GestureDetector(
      onTap: () {
        setState(() {
          signupas.indexnumber = index;
        });
      },
      child: Container(
        width: MediaQuery.of(context).size.width * 1 / 2.4,
        height: MediaQuery.of(context).size.height * 1 / 7,
        decoration: BoxDecoration(
          color: index == signupas.indexnumber
              ? Appcolors.pirimaryColor
              : Colors.white,
          borderRadius: BorderRadius.circular(4),
          boxShadow: [
            BoxShadow(
              spreadRadius: 0.5,
              color: Colors.grey.withOpacity(0.2),
              blurRadius: 1,
              offset: Offset(0, 1), // Shadow position
            ),
          ],
        ),
        child: Padding(
          padding: const EdgeInsets.all(5),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              SvgPicture.asset(
                assetName,
                width: 50,
                height: 50,
                color: index == signupas.indexnumber
                    ? Colors.white
                    : Appcolors.buttonColor,
              ),
              Text(
                data,
                style: TextStyle(
                  color: index == signupas.indexnumber
                      ? Colors.white
                      : Appcolors.pirimaryColor,
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
